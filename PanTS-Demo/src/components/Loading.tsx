...
const RotatingModelLoader: React.FC = () => {
    ...
    const [organ, setOrgan] = useState<number>(0);
  ...
  return (
    <div className="relative w-screen h-screen" ref={containerRef}>
      ...
      <div className="flex flex-col items-center justify-center gap-2 mt-64">
        <h2 className="text-2xl font-semibold text-center z-2">Preparing data...</h2>
        <div className="flex gap-2 z-2 items-center justify-between w-full opacity-50">
            <IconChevronLeft className="cursor-poiner z-2" onClick={() => setOrgan(organ === 0 ? 3 : organ - 1)}/>
            <h2 className="text-2xl font-semibold text-center z-2">{organ_arr[organ]}</h2>
            <IconChevronRight className="cursor-poiner z-2" onClick={() => setOrgan(organ === 3 ? 0 : organ + 1)}/>
        </div>
      </div>
    </div>
    </div>
