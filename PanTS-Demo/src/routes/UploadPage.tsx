...
const PIPELINE_INFO: Record<string, string> = {
  ...
  // Precision. FP16 routes inference to the quantized (half-precision) weights published
  // at eshanli/bodymaps-optimized-models; FP32 uses the original full-precision checkpoints.
  fp32: 'Full-precision (FP32) weights — the original checkpoints, with the largest memory footprint.',
  fp16: 'Quantized half-precision (FP16) weights from eshanli/bodymaps-optimized-models. Roughly half the VRAM and faster inference on the RTX 5070, with masks identical to FP32.',
};
...
const UploadPage: React.FC = () => {
  ...
  const processNiftiJob = async (job: UploadJob): Promise<void> => {
    ...
    try {
      ...
      inferFd.append('precision', job.precision);
      ...
      <div className="upload-main">
        <div className="upload-card">
          ...
          <div className="pipeline-row">
            ...
            <div className="pipeline-step">
              <div className="pipeline-step-header">
                <div className="pipeline-badge">4</div>
                <span className="pipeline-label">Precision</span>
              </div>
              <PipelineSelect
                value={selectedPrecision}
                onChange={setSelectedPrecision}
                placeholder="FP16 (quantized)"
                options={[
                  { value: 'fp16', label: 'FP16 (quantized)' },
